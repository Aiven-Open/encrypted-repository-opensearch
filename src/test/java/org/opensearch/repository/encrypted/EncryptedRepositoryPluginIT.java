...
public class EncryptedRepositoryPluginIT extends OpenSearchIntegTestCase {
    ...
    public void testFailsIfNoStorageTypeDefines() throws IOException {
        final RepositoryException exception = expectThrows(
                RepositoryException.class,
                () -> client().admin().cluster()
                        .preparePutRepository("enc-repo-no-storage-type")
                        .setType(EncryptedRepository.REPOSITORY_TYPE)
                        .get()
        );
        assertEquals(
                "[enc-repo-no-storage-type] failed to create repository",
                exception.getMessage()
        );
        assertEquals(
                "Setting storage_type hasn't been set. Supported are: [s3, fs, gcs, azure]",
    ...
    public void testFailsIfNonExistingStorageType() throws IOException {
        final RepositoryException exception = expectThrows(
                RepositoryException.class,
                () -> client().admin().cluster()
                        .preparePutRepository("enc-repo-wrong-storage-type")
                        .setType(EncryptedRepository.REPOSITORY_TYPE)
                        .setSettings(Settings.builder().put("storage_type", "aa"))
                        .get()
        );
        assertEquals(
                "[enc-repo-wrong-storage-type] failed to create repository",
                exception.getMessage()
        );
        assertEquals(
                "Unsupported storage type aa for storage_type. Supported are: [s3, fs, gcs, azure]",
                exception.getCause().getMessage()
        );
    }
    ...
    public void testCreateRepository() throws IOException {
        assertTrue(
                client().admin().cluster()
                        .preparePutRepository("enc-repo")
                        .setType(EncryptedRepository.REPOSITORY_TYPE)
                        .setSettings(
                                Settings.builder()
                                        .put(EncryptedRepositoryPlugin.STORAGE_TYPE_SETTING.getKey(), "fs")
                                        .put("location", "fs-data")
                        )
                        .execute()
                        .actionGet()
                        .isAcknowledged()
